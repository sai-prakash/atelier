...
function repoFromUrl(url) { const m = (url || "").match(/github\.com\/repos\/([^/]+\/[^/]+)/) || (url || "").match(/github\.com\/([^/]+\/[^/]+)/); return m ? m[1].replace(/\/(issues|pull)\/.*$/, "") : ""; }
...
if (state.token) { boot(state.token).catch(() => { state.user = null; render(); }); } else { render(); }
