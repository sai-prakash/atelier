const GH = {
  ...
  async readme(owner, repo) {
    try {
      const file = await this.req(`/repos/${owner}/${repo}/readme`);
  ...
  async dispatch(owner, repo, workflow, ref, inputs) {
    return this.req(`/repos/${owner}/${repo}/actions/workflows/${workflow}/dispatches`, {
      method: "POST",
      headers: { "Content-Type": "application/json" },
  ...
  async events(login) {
    return this.req(`/users/${login}/events?per_page=30`);
  },

  async pulls(owner, repo) {
    return this.req(`/repos/${owner}/${repo}/pulls?state=open&per_page=20`);
  },

  async languages(owner, repo) {
    try { return this.req(`/repos/${owner}/${repo}/languages`); }
    catch { return {}; }
  },
};
